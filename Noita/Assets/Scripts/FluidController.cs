...
public class FluidController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < randomFluidcount; i++)
            datas[UnityEngine.Random.Range(0, datas.Length)] = UnityEngine.Random.value;
    ...
    private void Update()
    {
        if(Input.anyKeyDown)
        {
            //int randomFluidcount = Mathf.RoundToInt(datas.Length / 10f);
            for (int i = 0; i < 10; i++)
        ...
        for (int i = 0; i < gridSize.x * gridSize.y * gridSize.z; i++)
        {
            ...
            for (int o = 0; o < adjacent.Count; o++)
            {
                    ...
                    temp.z >= 0 && temp.z < gridSize.z)
                {
                    ...
                    if (datas[rank] < 1f)
                        adjacentFluidValues.Add(new FluidQuantity(rank, datas[rank]));
                }
            }

            adjacentFluidValues.Sort((x, y) => x.quantity.CompareTo(y.quantity));
            adjacentFluidValues.Add(new FluidQuantity(0, 1f));

            for (int o = 0; o < adjacentFluidValues.Count - 1; o++)
            {
                if (adjacentFluidValues[o + 1].quantity > adjacentFluidValues[o].quantity)
                {
                    float count = o + 1;
                    float fluidDelta = Mathf.Min(fluidLeft, ((adjacentFluidValues[o + 1].quantity - adjacentFluidValues[o].quantity) * count));

                    for (int u = 0; u <= o; u++)
                    {
                        FluidQuantity temp = adjacentFluidValues[u];
                        adjacentFluidValues[u] = new FluidQuantity(temp.index, temp.quantity + fluidDelta / count);
                    }
                    fluidLeft -= fluidDelta;
                }
                ...
                if (fluidLeft <= 0f)
                {
                    adjacentFluidValues.RemoveAt(adjacentFluidValues.Count - 1);
                    //adjacentUnderFluidValues.RemoveRange(o + 1, adjacentUnderFluidValues.Count - (o-1));
                    break;
                }
            }

            for (int o = 0; o < adjacentFluidValues.Count; o++)
            {
                datas[adjacentFluidValues[o].index] = adjacentFluidValues[o].quantity;
            }
    ...
    private void OnDrawGizmosSelected()
    {
        if (datas != null)
        {
            for (int i = 0; i < datas.Length; i++)
            {
                if (datas[i] != 0f)
                {
                    Gizmos.DrawWireCube(CalculatePosition(i), datas[i] * Vector3.one);
                }
            }
        }
    }
